...
per commit (the in-tree transform as its pinned half, behind the immediate
shim; it witnesses the MODULE only), and the web build + boot is the
witness of record. Its shape is unchanged — a shader-shape change is
proven by witnesses, never by argument, and no witness substitutes for
another.
...
| `(file banner)` — COMPILER FLOOR block, `world.wgsl`, the Firefox clause (COMPAT_1, 2026-08-21) | PAID — differently | The priced regain was a generated no-immediates module beside the shipping one. COMPAT_1 ruled one module: the two immediates (`patch_params`, `shadow_slot`) returned to buffers as dynamic-offset uniform seats, the `requires` directive left the module, and naga reads it raw at the gate. Firefox and Safari are PENDING boot witnesses, owed in OPEN.md. |
| `(file banner)` — COMPILER FLOOR block, `world.wgsl`, the Firefox clause (ORGAN_8, 2026-08-21) | WITNESSED | Firefox (Windows): compiles with no diagnostics, boots, runs 30–56 fps. Safari 26 (iPad): 10+ min, transitions, smooth. The module has been read by all three Tint backends and by naga. Firefox's device loss after several transitions is a staging-memory matter — OPEN.md, FIREFOX STAGING RATCHET — and not a compiler one. |
