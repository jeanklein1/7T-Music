...
def main():
    ...
    if not exists(WEB_WGSL) and not exists(WEB_UNIFORMS):
        R("**Consequence for the campaign.** Both extra rooms are GONE.")
        R("`the_board`↔`the_chord` was already fossil; the desktop↔web axis was")
        R("live when this census was first taken and was **deleted in PRUNING_1")
        R("P1 Step 2** (ruling R1). A WGSL deletion is now genuinely a one-room")
        R("edit and a config-field deletion a two-room one, held by the")
        R("`offsetof` witnesses. The finding that made the web rooms dangerous —")
        R("that a resync is governed by BINDING CLOSURE, not entry-point")
        R("existence — is preserved in `audit/WEB_PORT_LEDGER.md`. Several §7")
        R("verdicts that read RULE(Jean) for mirror reasons are now unblocked.")
        R()
        R("---")
        R()
    else:
    ...
    R("### §3.3 — THE THIRD ROOM (`%s`) — %s" %
      (WEB_UNIFORMS, "⚠ LIVE" if web_u else "RESOLVED: deleted"))
    R()
    if web_u:
        ...
        R("**Resolved.** This room was a hand-indexed packer writing the same")
        R("592-byte config buffer by raw word index, checked by nothing. It was")
        R("deleted with the rest of the web port in **PRUNING_1 P1 Step 2**")
        R("(ruling R1). The config mirror is two rooms again — C++ and WGSL —")
        R("and the `offsetof` static_asserts in `%s` hold both." % STATE)
        R()
        R("Kept from it, in `audit/WEB_PORT_LEDGER.md`: all 21 of its")
        R("hand-annotated offsets agreed exactly with the offsets computed from")
        R("`state.hpp`. Authored independently, landing on the same numbers —")
        R("which is what made it a genuine third witness that §3.1's arithmetic")
        R("is right, and simultaneously what made deleting a config field")
        R("dangerous while it existed. **P3 is now a two-room edit.**")
        R()
    ...
    R("### §5.3 — the OTHER mirror law — %s" %
      ("NOT fossil ⚠" if exists(WEB_WGSL) else "RETIRED, and the code with it"))
    R()
    if not exists(WEB_WGSL):
        R("`web/shaders/world.wgsl` was a second, byte-identical copy of the")
        R("desktop shader under a **live** doctrine in `CLAUDE.md` — the one")
        R("real correction this census made to the P0 premise. Both the mirror")
        R("and the doctrine were deleted in **PRUNING_1 P1 Step 2** (ruling R1:")
        R("the port is paused, the doctrine is dead, not suspended).")
        R()
        R("**The finding outlived the code** and is recorded in")
        R("`audit/WEB_PORT_LEDGER.md`: a web host needed only 6 of the shader's")
        R("64 entry points, and the test governing a resync is **binding")
        R("closure, not entry-point existence**. At deletion the desktop")
        R("`patch_terrain_vs/fs` reached `g1:34 live_card_read`, which the")
        R("mirror never declared and the host's hand-written layout never")
        R("provided — the ritual as written would have failed at pipeline")
        R("creation while both entry points existed the whole time.")
        R()
        R("Consequence here: every §7 verdict whose only objection was \"live in")
        R("the web mirror\" is unblocked. A WGSL deletion is a one-room edit.")
        R()
    if exists(WEB_WGSL):
