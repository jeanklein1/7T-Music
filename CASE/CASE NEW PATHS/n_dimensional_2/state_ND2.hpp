 ...
 * ├─────────────────────────────────────────────────────────────────────────┤
 * │ Several constants in this file MUST match world.wgsl exactly:          │
 * │   • GPU struct layouts (GPUFrameSignal, GPUDesignConfig, etc.)         │
 * │   • Coupling bit definitions                                            │
 * │   • Field dynamics parameters (FIELD_ATTACK_*, FIELD_RELEASE_*, etc.)  │
 * │   • Texture/buffer sizes                                                │
 * │                                                                         │
 * │ The WGSL scroll is the single source of truth for GPU-side logic.      │
 * │ This file mirrors values needed for CPU-side initialization.           │
 * └─────────────────────────────────────────────────────────────────────────┘
 *
...
namespace t7 {
    namespace n_dimensional_2 {
        ...
        // ═══════════════════════════════════════════════════════════════════════════════
        //
        // This namespace contains two categories of values:
        //
        // TRAJECTORY IDLE VALUES (world.wgsl §1.1)
        //   Parameters that participate in attack/release dynamics.
        //   When stimulus ceases, these decay back to their idle value.
        //   Only these have corresponding constants in world.wgsl.
        //
        // INITIAL VALUES
        //   Starting positions for entities, config defaults.
        //   These don't participate in trajectory dynamics — they're either
        //   derived (pawn.y from terrain), accumulated (pawn.xz from input),
        //   or constant (sphere parameters).

        namespace Idle {
            // ─────────────────────────────────────────────────────────────────────────
            // TRAJECTORY IDLE VALUES — These match world.wgsl §1.1
            // ─────────────────────────────────────────────────────────────────────────
            //
            // These are the only true "idle" values — trajectory attractors that
            // stimulus-coupled parameters decay toward when stimulus is removed.

            // Terrain amplitude (stimulus: polyphony)
            // world.wgsl: IDLE_AMPLITUDE_SCALE
            constexpr float AMPLITUDE_SCALE = 1.0f;

            // Amplitude trajectory starts at idle with zero velocity
            constexpr float TRAJECTORY_VALUE = 1.0f;
            constexpr float TRAJECTORY_VELOCITY = 0.0f;

            // 2D field offset (stimulus: entity proximity)
            // world.wgsl: IDLE_FIELD_OFFSET
            constexpr float TRAJECTORY_FIELD_VALUE = 0.0f;
            constexpr float TRAJECTORY_FIELD_VELOCITY = 0.0f;

            // ─────────────────────────────────────────────────────────────────────────
            // INITIAL VALUES — Starting state, not trajectory attractors
            // ─────────────────────────────────────────────────────────────────────────

            // Ground parameters (constant, not trajectory-driven)
            constexpr float MAX_AMPLITUDE = 2.0f;
            constexpr float SIZE = 100.0f;
            constexpr float LIPSCHITZ_FACTOR = 1.0f;

            // Pawn starting position (derived/accumulated, not trajectory-driven)
            ...
            // Camera starting state (accumulated from input, not trajectory-driven)
            ...
            // Sphere parameters (derived from curve, not trajectory-driven)
        ...
        //
        // STIMULUS COUPLINGS (drive trajectory dynamics, have idle states):
        // ┌─────┬────────────────────────────────┬───────────────────────────────────────┐
        // │ Bit │ Coupling                       │ Flow                                  │
        // ├─────┼────────────────────────────────┼───────────────────────────────────────┤
        ...
        // └─────┴────────────────────────────────┴───────────────────────────────────────┘
        //
        // DERIVED COUPLINGS (computed from other state, no trajectory dynamics):
        // ┌─────┬────────────────────────────────┬───────────────────────────────────────┐
        ...
        // └─────┴────────────────────────────────┴───────────────────────────────────────┘
        //
        // ACCUMULATED COUPLINGS (integrate input over time, no trajectory dynamics):
        // ┌─────┬────────────────────────────────┬───────────────────────────────────────┐
        ...
        namespace Coupling {
            ...
            // By category
            constexpr uint32_t STIMULUS = POLYPHONY_TO_AMPLITUDE | PAWN_TO_FIELD_COLOR | SPHERE_TO_FIELD_COLOR;
            ...
            // By subsystem (legacy, still useful)
        ...
        struct alignas(16) GPUDesignConfig {
            ...
            uint32_t freeze_sphere;      // Freeze sphere at current position
        };
        ...
        struct alignas(16) GPUSphereState {
            float pos[3];
            float radius;
            float orientation[4];      // quaternion (derived from path tangent)
            float influence_radius;
            float t;                   // sphere's curve parameter (advances when not frozen)
            float _pad1;
            float _pad2;
        };
        ...
        static_assert(sizeof(GPUSphereState) == 48, "GPUSphereState must be 48 bytes");


        // ═══════════════════════════════════════════════════════════════════════════════
        // §4 CONSTANTS — Must match world.wgsl
        // ═══════════════════════════════════════════════════════════════════════════════
        //
        // These constants are used for CPU-side initialization and buffer sizing.
        // The GPU uses its own copies defined in world.wgsl.
        // Keep these synchronized manually.

        constexpr int MAX_TRAJECTORIES = 16;
        constexpr uint32_t HEIGHT_FIELD_SIZE = 256;   // world.wgsl: CHART_HEIGHT_RESOLUTION
        constexpr uint32_t TILE_TEXTURE_SIZE = 64;    // world.wgsl: CHART_TILE_RESOLUTION

        // ─── Trajectory Field (2D) ──────────────────────────────────────────────────
        // world.wgsl: TRAJECTORY_FIELD_SIZE, trajectory_field array size
        ...
        // world.wgsl: PAWN_INFLUENCE_RADIUS
        constexpr float PAWN_INFLUENCE_RADIUS = 8.0f;

        // ─── Field Dynamics ─────────────────────────────────────────────────────────
        // world.wgsl: FIELD_ATTACK_*, FIELD_RELEASE_*, FIELD_MAX_*
        ...
        private:
            ...
            bool initializeState() {
                ...
                // Orientation derived from tangent; at t=0 tangent is +Z, so identity quaternion
                ...
                sphere.orientation[0] = 0.0f;  // x
                sphere.orientation[1] = 0.0f;  // y
                sphere.orientation[2] = 0.0f;  // z
                sphere.orientation[3] = 1.0f;  // w (identity)
                sphere.influence_radius = Idle::SPHERE_INFLUENCE_RADIUS;
                sphere.t = 0.0f;               // curve parameter starts at 0
