...
/**
 * N-DIMENSIONAL_4 CARTRIDGE — GPU State Management
 * =================================================
 *
...
namespace t7 {
    namespace n_dimensional_4 {
        ...
        // Proximity field (for entity proximity effects)
        constexpr uint32_t PROXIMITY_FIELD_SIZE = 64;
        constexpr uint32_t PROXIMITY_FIELD_COUNT = PROXIMITY_FIELD_SIZE * PROXIMITY_FIELD_SIZE;  // 4096

        constexpr uint32_t TERRAIN_HEIGHT_FIELD_SIZE = 256;
        constexpr uint32_t SURFACE_COLOR_TEXTURE_SIZE = 64;  // Matches cell grid
        ...
        namespace Coupling {
            ...
            constexpr uint32_t ALL = 0x7FFFu;  // 15 bits now
            ...
            constexpr uint32_t DERIVED = TERRAIN_TO_PAWN_Y | TERRAIN_TO_PAWN_TILT | PAWN_TO_CAMERA_TARGET | TERRAIN_TO_SPHERE_HEIGHT;
        ...
        struct alignas(16) GPUDesignConfig {
            ...
            uint32_t fpv_mode;            // 0 = orbit camera, 1 = first-person view
            uint32_t wave_enable_mask;    // Bitmask: bit 0 = wave 0, bit 1 = wave 1, bit 2 = wave 2
            uint32_t wave_freeze_mask;    // Bitmask: frozen waves (captures time at freeze moment)
            float wave_frozen_t[3];       // Captured time for each frozen wave
            float _cfg_pad2;
        };
        ...
        struct alignas(16) GPUPawnState {
            ...
            float heading;
            float orientation[4];
            float velocity[2];    // Current XZ velocity (for force field radius)
        ...
        struct alignas(16) GPUCellState {
            ...
            float life;               // 4 bytes: Game of Life state (0.0 or 1.0)
            float life_next;          // 4 bytes: Next generation buffer (double-buffering)
            float _pad2;              // 4 bytes padding to align to 16
        ...
        static_assert(sizeof(GPUDesignConfig) == 64, "GPUDesignConfig must be 64 bytes");  // Was 48, added wave freeze
        static_assert(sizeof(GPUTrajectory) == 16, "GPUTrajectory must be 16 bytes");
        static_assert(sizeof(GPUTerrainState) == 32, "GPUTerrainState must be 32 bytes");  // Was 16, added tint
        static_assert(sizeof(GPUPawnState) == 48, "GPUPawnState must be 32 bytes");
        ...
        public:
            ...
            void set_fpv_mode(uint32_t mode) {
                config_.fpv_mode = mode;
            }

            uint32_t get_fpv_mode() const {
                return config_.fpv_mode;
            }

            void toggle_wave(uint32_t wave_index) {
                if (wave_index < 3) {
                    config_.wave_enable_mask ^= (1u << wave_index);
                }
            }

            void set_wave_enabled(uint32_t wave_index, bool enabled) {
                if (wave_index < 3) {
                    if (enabled) {
                        config_.wave_enable_mask |= (1u << wave_index);
                    } else {
            ...
            bool is_wave_enabled(uint32_t wave_index) const {
                return (config_.wave_enable_mask & (1u << wave_index)) != 0;
            }

            uint32_t get_wave_enable_mask() const {
                return config_.wave_enable_mask;
            }

            // ─── Wave Freeze Control ──────────────────────────────────────────────
            //
            // Freezing a wave captures the current time and holds the wave at that
            // position. Unlike disabling, the wave remains visible but stops moving.

            void toggle_wave_freeze(uint32_t wave_index, float current_wave_time) {
                if (wave_index < 3) {
                    uint32_t bit = 1u << wave_index;
                    if (config_.wave_freeze_mask & bit) {
                        // Unfreeze
                        config_.wave_freeze_mask &= ~bit;
                    } else {
                        // Freeze: capture current time
                        config_.wave_freeze_mask |= bit;
                        config_.wave_frozen_t[wave_index] = current_wave_time;
                    }
                }
            }

            void set_wave_frozen(uint32_t wave_index, bool frozen, float wave_time) {
                if (wave_index < 3) {
                    if (frozen) {
                        config_.wave_freeze_mask |= (1u << wave_index);
                        config_.wave_frozen_t[wave_index] = wave_time;
                    } else {
                        config_.wave_freeze_mask &= ~(1u << wave_index);
                    }
                }
            ...
            bool is_wave_frozen(uint32_t wave_index) const {
                return (config_.wave_freeze_mask & (1u << wave_index)) != 0;
            }

            uint32_t get_wave_freeze_mask() const {
                return config_.wave_freeze_mask;
            }

            float get_wave_frozen_t(uint32_t wave_index) const {
                if (wave_index < 3) {
                    return config_.wave_frozen_t[wave_index];
                }
                return 0.0f;
            }
            ...
            static constexpr uint32_t terrain_height_field_workgroups() {
                return TERRAIN_HEIGHT_FIELD_SIZE / 8;
            }
            ...
            static constexpr uint32_t surface_color_workgroups() {
                return SURFACE_COLOR_TEXTURE_SIZE / 8;
            }
        ...
        private:
            ...
            wgpu::Buffer terrainProximityFieldBuffer_;
            wgpu::Buffer terrainCellsBuffer_;

            // ─── Textures ────────────────────────────────────────────────────────────
            wgpu::Texture terrainHeightFieldTexture_;
            wgpu::TextureView terrainHeightFieldWriteView_;
            wgpu::TextureView terrainHeightFieldReadView_;

            wgpu::Texture terrainSurfaceColorTexture_;
            wgpu::TextureView terrainSurfaceColorWriteView_;
            wgpu::TextureView terrainSurfaceColorReadView_;

            wgpu::Texture cellHeightFieldTexture_;
            wgpu::TextureView cellHeightFieldWriteView_;
            wgpu::TextureView cellHeightFieldReadView_;
            ...
            bool createBuffers() {
                ...
                // Terrain proximity field buffer (binding 7) - for entity proximity effects
                {
                    wgpu::BufferDescriptor desc{};
                    desc.label = "Terrain Proximity Field";
                    desc.size = sizeof(GPUTrajectory) * PROXIMITY_FIELD_COUNT;
                    desc.usage = wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopyDst;
                    terrainProximityFieldBuffer_ = device_.CreateBuffer(&desc);
                    if (!terrainProximityFieldBuffer_) return false;
                }

                // Terrain cells buffer (binding 8)
                {
                    wgpu::BufferDescriptor desc{};
                    desc.label = "Terrain Cell States";
                    desc.size = sizeof(GPUCellState) * CELL_COUNT;
                    desc.usage = wgpu::BufferUsage::Storage | wgpu::BufferUsage::CopyDst;
                    terrainCellsBuffer_ = device_.CreateBuffer(&desc);
                    if (!terrainCellsBuffer_) return false;
                }
            ...
            bool createTextures() {
                // Terrain height field texture
                {
                    wgpu::TextureDescriptor desc{};
                    desc.label = "Terrain Height Field";
                    desc.size = { TERRAIN_HEIGHT_FIELD_SIZE, TERRAIN_HEIGHT_FIELD_SIZE, 1 };
                    desc.format = wgpu::TextureFormat::RGBA16Float;
                    desc.usage = wgpu::TextureUsage::StorageBinding | wgpu::TextureUsage::TextureBinding;
                    terrainHeightFieldTexture_ = device_.CreateTexture(&desc);
                    if (!terrainHeightFieldTexture_) return false;

                    terrainHeightFieldWriteView_ = terrainHeightFieldTexture_.CreateView();
                    terrainHeightFieldReadView_ = terrainHeightFieldTexture_.CreateView();
                }

                // Terrain surface color texture (renderer observes cell colors here)
                {
                    wgpu::TextureDescriptor desc{};
                    desc.label = "Terrain Surface Color";
                    desc.size = { SURFACE_COLOR_TEXTURE_SIZE, SURFACE_COLOR_TEXTURE_SIZE, 1 };
                    desc.format = wgpu::TextureFormat::RGBA8Unorm;
                    desc.usage = wgpu::TextureUsage::StorageBinding | wgpu::TextureUsage::TextureBinding;
                    terrainSurfaceColorTexture_ = device_.CreateTexture(&desc);
                    if (!terrainSurfaceColorTexture_) return false;

                    terrainSurfaceColorWriteView_ = terrainSurfaceColorTexture_.CreateView();
                    terrainSurfaceColorReadView_ = terrainSurfaceColorTexture_.CreateView();
                }

                // Cell height field texture (cell-driven geometry, separate from waves)
                {
                    wgpu::TextureDescriptor desc{};
                    desc.label = "Cell Height Field";
                    desc.size = { CELL_GRID_SIZE, CELL_GRID_SIZE, 1 };
                    desc.format = wgpu::TextureFormat::R32Float;
                    desc.usage = wgpu::TextureUsage::StorageBinding | wgpu::TextureUsage::TextureBinding;
                    cellHeightFieldTexture_ = device_.CreateTexture(&desc);
                    if (!cellHeightFieldTexture_) return false;

                    cellHeightFieldWriteView_ = cellHeightFieldTexture_.CreateView();
                    cellHeightFieldReadView_ = cellHeightFieldTexture_.CreateView();
                }
            ...
            bool createBindGroups() {
                ...
                // ═══════════════════════════════════════════════════════════════════
                {
                    ...
                    // @binding(1) config (needed by fragment shader for active_cell_size)
                    entries[0].binding = 1;
                    entries[0].visibility = wgpu::ShaderStage::Fragment;
                    entries[0].buffer.type = wgpu::BufferBindingType::Uniform;

                    // @binding(10) render_signal
                    entries[1].binding = 10;
                    entries[1].visibility = wgpu::ShaderStage::Fragment;
                    entries[1].buffer.type = wgpu::BufferBindingType::ReadOnlyStorage;

                    // @binding(11) render_ground
                    entries[2].binding = 11;
                    entries[2].visibility = wgpu::ShaderStage::Fragment;
                    entries[2].buffer.type = wgpu::BufferBindingType::ReadOnlyStorage;

                    // @binding(12) render_pawn
                    entries[3].binding = 12;
                ...
                // ═══════════════════════════════════════════════════════════════════
                {
                    ...
                    // @binding(2) cell_height_field_write
                    entries[2].binding = 2;
                    entries[2].visibility = wgpu::ShaderStage::Compute;
                ...
                // ═══════════════════════════════════════════════════════════════════
                {
                    ...
                    // @binding(14) cell_height_field_read (R32Float is unfilterable)
                ...
                // Compute entity bind group
                {
                    ...
                    entries[2].binding = 2;
                    entries[2].buffer = terrainBuffer_;
                    entries[2].size = sizeof(GPUTerrainState);
                    ...
                    entries[7].binding = 7;
                    entries[7].buffer = terrainProximityFieldBuffer_;
                    entries[7].size = sizeof(GPUTrajectory) * PROXIMITY_FIELD_COUNT;

                    entries[8].binding = 8;
                    entries[8].buffer = terrainCellsBuffer_;
                ...
                // Compute texture bind group
                {
                    ...
                    entries[0].binding = 0;
                    entries[0].textureView = terrainHeightFieldWriteView_;

                    entries[1].binding = 1;
                    entries[1].textureView = terrainSurfaceColorWriteView_;

                    entries[2].binding = 2;
                    entries[2].textureView = cellHeightFieldWriteView_;
                ...
                // Render texture bind group
                {
                    ...
                    entries[0].binding = 10;
                    entries[0].textureView = terrainHeightFieldReadView_;

                    entries[1].binding = 11;
                    entries[1].textureView = terrainSurfaceColorReadView_;
                    ...
                    entries[4].binding = 14;
                    entries[4].textureView = cellHeightFieldReadView_;
            ...
            bool initializeState() {
                ...
                config_.wave_freeze_mask = 0;    // No waves frozen
                ...
                terrain.tint[0] = 1.0f;  // Neutral tint at start
                terrain.tint[1] = 1.0f;
                terrain.tint[2] = 1.0f;
                terrain._pad = 0.0f;
                ...
                // ─── Terrain Proximity Field ──────────────────────────────────────────
                // Initialize all cells to idle (0.0 value, 0.0 velocity)
                std::vector<GPUTrajectory> proximityField(PROXIMITY_FIELD_COUNT);
                for (uint32_t i = 0; i < PROXIMITY_FIELD_COUNT; ++i) {
                    proximityField[i].value = 0.0f;     // IDLE_FIELD_OFFSET
                    proximityField[i].velocity = 0.0f;
                    proximityField[i]._pad0 = 0.0f;
                    proximityField[i]._pad1 = 0.0f;
                }
                queue.WriteBuffer(terrainProximityFieldBuffer_, 0, proximityField.data(),
                    sizeof(GPUTrajectory) * PROXIMITY_FIELD_COUNT);
                ...
                for (uint32_t y = 0; y < CELL_GRID_SIZE; ++y) {
                    for (uint32_t x = 0; x < CELL_GRID_SIZE; ++x) {
                        ...
                        // Game of Life: seed with ~20% random alive cells
                        // Using simple hash based on position for deterministic pattern
                        uint32_t hash = (x * 374761393u + y * 668265263u) ^ (x * y);
                        cell.life = (hash % 5 == 0) ? 1.0f : 0.0f;
                        cell.life_next = cell.life;  // Initialize buffer to match

                        cell._pad2 = 0.0f;
                    }
                }
                queue.WriteBuffer(terrainCellsBuffer_, 0, cells.data(), sizeof(GPUCellState) * CELL_COUNT);
