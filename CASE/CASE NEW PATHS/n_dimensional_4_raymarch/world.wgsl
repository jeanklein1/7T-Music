...
struct DesignConfig {
    ...
    fpv_mode: u32,            // 0 = orbit camera, 1 = first-person view
    wave_enable_mask: u32,    // Bitmask: bit 0 = wave 0, bit 1 = wave 1, bit 2 = wave 2
    wave_freeze_mask: u32,    // Bitmask: frozen waves (captures time at freeze moment)
    wave_frozen_t: array<f32, 3>,  // Captured time for each frozen wave
    _cfg_pad2: f32,
}
...
const CELL_RANDOM_TEMPORAL_PATTERN_CHANGE: bool = true;
...
const CELL_RANDOM_DEBUG_OSC_SPEED: f32 = 0.001;
...
const CELL_HEIGHT_ENABLED: bool = true;      // Master switch for cell height geometry
...
fn wave_frozen(index: i32) -> bool {
    return (config.wave_freeze_mask & (1u << u32(index))) != 0u;
}

fn wave_time_for(index: i32, global_t: f32) -> f32 {
    // Returns the time to use for this wave:
    // - If frozen: use the captured time from when it was frozen
    // - If not frozen: use the current global time
    if (wave_frozen(index)) {
        return config.wave_frozen_t[index];
    }
    return global_t;
}
...
fn dynamics_terrain_wave_eval(pos: vec2<f32>, t: f32, amplitude_scale: f32) -> f32 {
    ...
    for (var i: i32 = 0; i < WAVE_COUNT; i++) {
        if (wave_enabled(i)) {
            let wave_t = wave_time_for(i, t);
            h += dynamics_terrain_wave_single(pos, wave_t, WAVES[i]);
        }
    }
...
fn dynamics_terrain_gradient_eval(pos: vec2<f32>, t: f32, amplitude_scale: f32) -> vec2<f32> {
    ...
    for (var i: i32 = 0; i < WAVE_COUNT; i++) {
        if (wave_enabled(i)) {
            let wave_t = wave_time_for(i, t);
            grad += dynamics_terrain_gradient_single(pos, wave_t, WAVES[i]);
        }
    }
...
fn compose_entities_0d_update(dt: f32, t: f32) {
    ...
    if (coupling_active(COUPLING_INPUT_MOVES_PAWN)) {
        ...
        // Pawn heading: FPV = follow camera, Orbit = follow velocity
        if (fpv_mode_active()) {
        ...
        // In FPV mode, pawn still follows camera even when stationary
        if (fpv_mode_active()) {
            pawn.heading = camera_state.azimuth;
        }
    }
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // WIRING: Input → Camera (orbit, zoom, pan) — Mode-aware
    // ═══════════════════════════════════════════════════════════════════════
    
    var camera = camera_state;
    
    // Camera rotation always active, but elevation limits differ by mode
    if (coupling_active(COUPLING_INPUT_ORBITS_CAMERA)) {
        camera.azimuth += signal.look_az_delta;
        
        // Mode-dependent elevation limits
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // WIRING: Pawn → Camera (follow target) — Mode-aware positioning
    // ═══════════════════════════════════════════════════════════════════════
    
    if (fpv_mode_active()) {
        // FPV: Camera at eye level, directly at pawn position
        camera.pos = pawn_state.pos + vec3(0.0, FPV_EYE_HEIGHT, 0.0);
    } else if (coupling_active(COUPLING_PAWN_TO_CAMERA_TARGET)) {
        // Orbit: Camera orbits around pawn
