...
/**
 * DESIGN CARTRIDGE 2 — Render Cartridge
 * ======================================
 * 
 * Extension of design_cartridge with per-texel trajectory field.
 * 
 ...
 *   5. Compute: evolve_trajectory_field   [2D] — NEW
 *   6. Compute: update_tiles              [2D]
 *   7. Swap trajectory field ping-pong    — NEW
 *   8. Render:  fullscreen raymarch
 * 
 * NEW IN CARTRIDGE 2:
 *   - Trajectory field: 256×256 per-texel activation
 *   - Stimulus: polyphony (global) + pawn presence (local)
 *   - Evolution: spring attack, exponential decay
 *   - Effect: tiles blend between idle and active colors
 * 
 * DESIGN MODE:
...
#include "cartridges/design_cartridge_2/renderer.hpp"
#include <cmath>
#include <iostream>
...
public:
    ...
    //   3. Compute: evolve_trajectory_field   [2D] — activation texture (NEW)
    //   4. Compute: update_tiles              [2D] — tile colors (reads activation)
    //   5. Swap trajectory field ping-pong
    ...
    void render(wgpu::CommandEncoder& encoder,
        ...
        // ─── COMPUTE PHASE ──────────────────────────────────────────────────
        {
            ...
            // Pass 3: Evolve trajectory field (2D) — NEW
            //
            // Per-texel activation based on polyphony + pawn presence.
            // Reads from previous frame's texture, writes to next frame's.
            renderer_.dispatch_evolve_trajectory_field(
                compute,
                gpuState_.compute_entity_bind_group(),
                gpuState_.compute_texture_bind_group(),
                GPUState::trajectory_field_workgroups()
            );
            
            // Pass 4: Update tiles (2D)
            //
            // Samples trajectory field to blend tile colors.
        ...
        // Swap trajectory field for next frame — NEW
        //
        // The compute_texture_bind_group() getter returns A or B based on this flag.
        // A reads A→writes B, B reads B→writes A.
        gpuState_.swap_trajectory_field();
    ...
    void isolate_terrain_coupling() {
        gpuState_.set_mute_couplings(~Coupling::TERRAIN);
    }
    ...
    void isolate_input_coupling() {
        gpuState_.set_mute_couplings(~Coupling::INPUT);
    }
    ...
    void isolate_signal_coupling() {
        gpuState_.set_mute_couplings(~Coupling::SIGNAL);
    }
