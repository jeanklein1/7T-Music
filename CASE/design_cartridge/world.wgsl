// ═══════════════════════════════════════════════════════════════════════════════
// DESIGN CARTRIDGE — GPU Scroll
// ═══════════════════════════════════════════════════════════════════════════════
//
// A pawn traversing a musically-reactive wave terrain.
//
// ┌─────────────────────────────────────────────────────────────────────────────┐
// │ MANIFEST                                                                    │
// ├─────────────────────────────────────────────────────────────────────────────┤
// │ Chart:     Plane, 100×100 world units, 256² texels                          │
// │ Fields:    height [WaveSum], tile_color [Static]                            │
// │ Entities:  pawn [RoundCone], camera [Orbital]                               │
// │ Couplings: 7 total                                                          │
...
//   §3  FIELDS ............................ line 184
//   §4  ENTITIES .......................... line 270
//   §5  COUPLINGS ......................... line 365
//   §6  COMPOSE ........................... line 503
//   §7  RENDER ............................ line 631
//   §8  BINDINGS .......................... line 790
//   §9  ENTRY POINTS ...................... line 844
//
// ═══════════════════════════════════════════════════════════════════════════════
...
// Idle:     Animated waves at base amplitude (amplitude_scale = 1.0)
...
// Dynamics: Static (pattern-based)
// Idle:     checkerboard of warm sand and near-black

const TILE_COLOR_A: vec3<f32> = vec3(0.88, 0.78, 0.73);
const TILE_COLOR_B: vec3<f32> = vec3(0.05, 0.05, 0.05);

fn field_tile_color(tile_id: vec2<u32>) -> vec4<f32> {
    let is_dark = ((tile_id.x + tile_id.y) % 2u) == 0u;
    let color = select(TILE_COLOR_A, TILE_COLOR_B, is_dark);
    let activation = 1.0;
    return vec4(color, activation);
}
...
fn compose_update_tiles(texel: vec2<u32>) {
    let color = field_tile_color(texel);
    textureStore(tile_state_write, texel, color);
}
...
fn sdf_scene(p: vec3<f32>) -> SceneHit {
    ...
    if (in_bounds) {
        ...
        if (d_ground < d_pawn) {
            return SceneHit(d_ground, MAT_GROUND);
        }
    }
    
    return SceneHit(d_pawn, MAT_PAWN);
}
...
@group(0) @binding(5) var<storage, read_write> trajectories: array<Trajectory, 16>;
