...
inline void release_finite_ribbons(RibbonState& rs, RibbonDeps* c, wgpu::Queue& queue) {
    if (c->world_state_.finite_mode && rs.active_count > 0 && !MOOD_TABLE[c->mood_state_.active].has_anchor_ribbon) {
        for (uint32_t i = 0; i < MAX_RIBBON_INSTANCES; i++) {
            rs.active[i] = ActiveRibbon{};
            rs.gpu[i] = GPURibbonState{};
        }
...
inline void promote_ribbon_to_rendered(RibbonState& rs, RibbonDeps* c, uint32_t slot, wgpu::Queue& queue) {
