...
struct DriverSurface {
    ...
    // THE GROUND'S VOICE stood here (GROUND_VOICE_0 → RELIEF_1): two
    // gains, the lift and the quickening, whose rests lived in the
    // driven fields themselves (config's two gol scales, boot-pinned
    // 1.0 and still hand dials). Excised by ruling — one author per
    // fact; the ground's two remaining gains are the strike ring's and
    // the relief's.
    struct Ground {
