...
namespace the_board {
    ...
    inline void upload_automaton_header(AutomatonState& as, AutomatonDeps* c,
                                        wgpu::Queue& queue) {
        // THE TICK SCALE REACHES THE STEP HERE, AND ONLY HERE — as a
        // HAND DIAL (RELIEF_1). GROUND_VOICE_0 first gave it a reach
        // (its density quicken multiplied the period here); RELIEF_1
        // excised that coupling because a scale that is not a divisor of
        // the bar moves the tick off the bar line, and RELIEF_0's latch
        // rides the tick. The dial remains: a hand that sets 0.7 knows
        // it has left the bar; a coupling did it silently. The gate is
        // floor(beats / period) against a stored index, so any
        // CONTINUOUS driver aimed at this field would fire steps between
        // bar lines — a future one wants a subdivision ladder (bar,
        // half, quarter), which is new mechanism, not a rescue.
        //
