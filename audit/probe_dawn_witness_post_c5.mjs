// [CONTACT_4 S3] Dawn validation witness — POST-BATCH variant (CONTACT_2 — the flee/steer/plasticity kernels revalidate; zero new bindings).
//
// Same method as probe_dawn_witness.mjs (headless Chromium = real Dawn), but
// against the BATCH tree: layouts re-parsed into audit/cc6_output_post_c5.json;
...
// edit landed coherently. Writes audit/probe_results_post_c5.json.
//
...
const cc6 = JSON.parse(readFileSync('audit/cc6_output_post_c5.json', 'utf8'));
...
const result = await page.evaluate(async ({ wgsl, layouts, families }) => {
  ...
  const module = device.createShaderModule({ code: wgsl, label: 'world.wgsl (post-C5 witness)' });
...
writeFileSync('audit/probe_results_post_c5.json', JSON.stringify(result, null, 2));
const flat = [];
