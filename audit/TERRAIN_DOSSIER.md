...
| `CONTRIB_PAINTINGS_BASES` (:107) | `contrib_paintings_base_at`:2305 — returns 0.0, **zero call sites, present in no policy mask** (neither C++ :176–252 nor WGSL :2120–2151) | DECLARED-ONLY (stub fn exists, dead) |
| `CONTRIB_VEGETATION_BASES` (:108) | `contrib_vegetation_base_at`:2315 — same: 0.0 stub, zero call sites, in no mask | DECLARED-ONLY (stub fn exists, dead) |
...
| PYRAMIDS → PAINTINGS_BASES (:150), SOLIDS → PAINTINGS_BASES (:151) | `to` endpoint is a 0.0 stub in no policy mask — closure asserts pass vacuously, no runtime composition exists | DECLARED-ONLY |
| SOLIDS → VEGETATION_BASES (:152) | same — vacuous | DECLARED-ONLY |

**The `CONTRIBUTOR_DAG` table itself has no reader.** The closure machinery `ASSERT_POLICY_DAG_CLOSED` (:275–293) hardcodes the same 6 edges as literal enum pairs rather than iterating the table, and `CONTRIBUTOR_DAG_EDGE_COUNT` (:154–155) is referenced nowhere. Adding an edge to `CONTRIBUTOR_DAG[]` validates nothing until the macro is separately edited — world.wgsl's extension checklist even says so ("update ASSERT_POLICY_DAG_CLOSED's edge list", :2252). The declared table is documentation-shaped data; the assert block is its hand-kept mirror.

#### Policy rows → realization

| Declaration row (.inl) | WGSL mask | Query fn | Live consumers | Verdict |
|---|---|---|---|---|
| `POLICY_PLACEMENT_PYRAMID` (:179–181) | :2120 | `query_ground_placement_pyramid`:2593 — **zero call sites** | live pyramid Y-correction is `compute_entity_placement` 5-point min over the baked heightfield (:7977–7997), whose set **includes CONTRIB_PYRAMIDS** — the declared mask excludes it ("pyramids don't see themselves", :2592) | DECLARED-ONLY (fn dead; live path uses a different contributor set) |
| `POLICY_PLACEMENT_PAINTING` (:183–187) | :2121–2123 | `query_ground_placement_painting`:2602 — zero call sites | realized as a documented hybrid: `sample_terrain_y_at + contrib_gol_zones_at` (:7913–7914, equivalence argued :7899–7912) in `compute_entity_placement`:7887 | REALIZED (via hybrid; named fn dead) |
| `POLICY_PLACEMENT_VEGETATION` (:189–191) | :2124 | `query_ground_placement_vegetation`:2615 — zero call sites | live palm/cactus/blade/column Y path samples the baked heightfield (:7930, 7939, 7949, 7959) = static_base **+ pyramids**, contradicting the declared "trees don't stand on pyramids" mask (:190) | DECLARED-ONLY (fn dead; live path diverges from declared set) |
...
| `POLICY_FLYER` (:202–209, gradient=true) | :2127–2132 | `query_ground_flyer`:2645 | sphere `coupling_terrain_to_sphere_orbit_height`:2867, `update_camera`:6279, `update_cube`:6570/6575/6617 | REALIZED — but `query_ground_flyer_gradient`:2802 has zero callers (declared gradient support is dead code) |
| `POLICY_WALKER` (:214–222, gradient=true) | :2133–2139 | `query_ground_walker`:2678; fused `query_ground_walker_pair`:2743 | `pawn_ground_resolve` :5385–5405, `behavior_player_controlled` :5590–5593 | REALIZED — but `query_ground_walker_gradient`:2816 and `query_ground_walker_walkable`:2833 have zero callers |
| `POLICY_WALKER_TILT` (:230–236, gradient=true) | :2140–2144 | `query_ground_walker_tilt`:2713 | `terrain_normal_at`:5348–5352 (hand-rolled 3-sample gradient; no `_gradient` fn per the :2267 convention), `query_ground_walker_pair`:2758 | **REALIZED-DIVERGENT** — declared mask (both .inl and WGSL) omits `CONTRIB_GOL_SUPPRESSION`, yet the body applies pawn-centered GoL suppression (:2718–2721; also pair :2755–2757). The .inl calls it "walker minus … suppression" (:124, 224–229); even world.wgsl's own consumer comment repeats the stale claim (:5343–5344) against the body it calls |
| `POLICY_WALKER_AGENT` (:239–246, gradient=true) | :2145–2150 | `query_ground_walker_agent`:2775 | `agent_post_step`:5486 (scalar only) | REALIZED — but `gradient_supported=true` has **no realization anywhere**: no gradient fn, no multi-sample consumer |
| `POLICY_CELESTIAL` (:249–251) | :2151 | `query_ground_celestial`:2791 — zero callers, "kept for symmetry" (:2790) | none ("future celestial entity placement (none today)", :2789) | DECLARED-ONLY (self-documented symmetry stub) |

Note: world.wgsl:7877–7879 claims the analytical `query_ground_placement_*` functions are "used by the real spawn decision code (e.g. patch entity proposal / spawn engines in cartridge.hpp)" — no such consumer exists; C++ cannot call WGSL, and the CPU deliberately stays on `estimate_terrain_height` ("NOT a ground policy query", spawn_engine.inl:1116–1125). All three placement queries are dead code and the comment points at a phantom caller.
...
| `patch_terrain_vs` :3494–3510 | hand-fused render path: heightfield + aura(:3557) + pulses(:3565) + waves — **explicitly no GoL** (:3502) | CODE-WITHOUT-ROW — its contributor set matches no declared policy; self-labeled "POLICY_FLYER-ish" with a keep-consistent obligation (:3509–3510) enforced by nothing |
...
| ~15 render VS sites adding `contrib_terrain_waves_at` alone (:4083–4154, 8525, 8577, 9958, 9972, 10288, 10302, 10520, 10534) | single-contributor inline consumption | CODE-WITHOUT-ROW, uniform pattern |
...
Of 11 contributor rows: 8 REALIZED (one driver-parked, one dual-form, one inline-only), 2 DECLARED-ONLY stubs, 1 (SUPPRESSION) realized only outside its named function. Of 6 DAG edges: 3 REALIZED, 3 vacuous DECLARED-ONLY — and the edge table itself is read by nothing. Of 9 policy rows: 5 REALIZED, 3 DECLARED-ONLY (both non-painting placement rows diverge from the live path's contributor set), 1 REALIZED-DIVERGENT (`walker_tilt`, the single row where the declared mask and the shipped body compute different contributor sets). The registry's compile-time closure machinery is sound for what it checks, but it checks a hand-copied edge list against masks that no runtime — GPU or CPU — ever reads.
...
Six of the twelve are ground dials. This is L0's future musical surface:

#### The six ground dials
