// [GROUND_CARD_1 H6] Dawn validation witness — POST-BATCH variant.
//
// Same method as probe_dawn_witness.mjs (headless Chromium = real Dawn), but
// against the BATCH tree: layouts re-parsed into audit/cc6_output_post_ug1.json,
// families updated to the batch renderer (placement gains the Compute Texture
// group; the Live Card Writer family is new). BASELINE ONLY — the eviction
// probes are moot post-eviction (the removals ARE the tree now); a green
// baseline here witnesses that every paired layout/group/shader edit landed
// coherently. Writes audit/probe_results_post_ug1.json.
//
// Usage: node audit/probe_dawn_witness_post_gc1.mjs   (from repo root)
...
const cc6 = JSON.parse(readFileSync('audit/cc6_output_post_ug1.json', 'utf8'));
...
const FAMILIES = {
  ...
  live_card: { groups: ['Live Card Writer Layout'], eps: ['write_live_card'] },
...
writeFileSync('audit/probe_results_post_ug1.json', JSON.stringify(result, null, 2));
const flat = [];
